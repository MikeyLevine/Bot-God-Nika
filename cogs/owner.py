...
class Owner(commands.Cog, name="owner"):
  ...
  async def sync(self, context: Context, scope: str) -> None:
    ...
    if scope == "global":
      ...
      await context.send(embed=embed)
      return
    embed = discord.Embed(title="Invalid Scope",
  ...
  async def blacklist_add(self, context: Context, user: discord.User) -> None:
    ...
    embed.set_footer(
      text=
      f"There are now {total} {'user' if total == 1 else 'users'} in the blacklist"
    )
    await context.send(embed=embed)
  ...
  @checks.is_owner()
  async def blacklist_remove(self, context: Context,
